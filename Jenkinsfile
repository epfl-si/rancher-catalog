...
node('docker') {
  try {
    ...
    lock("template-${projectname}-${env.BRANCH_NAME}") {
      ...
      template_pipeline.deploy_test_and_cleanup(rancher_server_url, rancher_server_credid, rancher_env_id, projectname, isinfratemplate, stack_env_100, this.&tests)
      template_pipeline.deploy_test_and_cleanup(rancher_server_url, rancher_server_credid, rancher_env_id, projectname, isinfratemplate, stack_env_101, this.&tests)
